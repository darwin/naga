...
ObjectTracer2::ObjectTracer2(v8::Local<v8::Value> handle, pb::object* object)
    : m_handle(v8::Isolate::GetCurrent(), handle), m_object(object), m_living(GetLivingMapping()) {}
...
void ObjectTracer2::Dispose(void) {
  // m_handle.ClearWeak();
  m_handle.Reset();
}

ObjectTracer2& ObjectTracer2::Trace(v8::Local<v8::Value> handle, pb::object* object) {
...
/*
void ObjectTracer2::WeakCallback(const v8::WeakCallbackInfo<ObjectTracer2>& info)
{
  std::unique_ptr<ObjectTracer2> tracer(info.GetParameter());

  // assert(info.GetValue() == tracer->m_handle);
}
*/

LivingMap2* ObjectTracer2::GetLivingMapping(void) {
  v8::HandleScope handle_scope(v8::Isolate::GetCurrent());
...
v8::Local<v8::Value> ObjectTracer2::FindCache(pb::handle obj) {
  LivingMap2* living = GetLivingMapping();

  if (living) {
    LivingMap2::const_iterator it = living->find(obj.ptr());
...
ContextTracer2::~ContextTracer2(void) {
  v8::HandleScope handle_scope(v8::Isolate::GetCurrent());
...
void ContextTracer2::Trace(void) {
  m_ctxt.SetWeak(this, WeakCallback, v8::WeakCallbackType::kFinalizer);
}