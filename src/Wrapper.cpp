...
#else
    error->ToObject(ctxt).ToLocalChecked()->SetPrivate(ctxt, privateKey_type,
                                                       v8::External::New(isolate, new py::object(type)));
    error->ToObject(ctxt).ToLocalChecked()->SetPrivate(ctxt, privateKey_value,
...
#define TRY_HANDLE_EXCEPTION(value)          \
  _TERMINATE_CALLBACK_EXECUTION_CHECK(value) \
...
void CPythonObject::NamedGetter(v8::Local<v8::Name> prop, const v8::PropertyCallbackInfo<v8::Value>& info) {
  ...
  if (!value) {
    ...
    CALLBACK_RETURN(v8::Local<v8::Value>());
  }
...
void CPythonObject::NamedQuery(v8::Local<v8::Name> prop, const v8::PropertyCallbackInfo<v8::Integer>& info) {
  if (prop->IsSymbol()) {
    // ignore symbols for now, see https://github.com/area1/stpyv8/issues/8
    CALLBACK_RETURN(v8::Local<v8::Integer>());
  }
  v8::HandleScope handle_scope(info.GetIsolate());
...
void CPythonObject::NamedDeleter(v8::Local<v8::Name> prop, const v8::PropertyCallbackInfo<v8::Boolean>& info) {
  if (prop->IsSymbol()) {
    // ignore symbols for now, see https://github.com/area1/stpyv8/issues/8
    CALLBACK_RETURN(v8::Local<v8::Boolean>());
  }
  v8::HandleScope handle_scope(info.GetIsolate());
...
void CPythonObject::NamedEnumerator(const v8::PropertyCallbackInfo<v8::Array>& info) {
  ...
  if (::PySequence_Check(obj.ptr())) {
    CALLBACK_RETURN(v8::Local<v8::Array>());
  ...
  v8::Local<v8::Array> result = v8::Array::New(info.GetIsolate(), len);
  ...
  if (len > 0) {
    for (Py_ssize_t i = 0; i < len; i++) {
      ...
      result->Set(v8::Isolate::GetCurrent()->GetCurrentContext(), v8::Uint32::New(info.GetIsolate(), i),
...
void CPythonObject::Caller(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  switch (info.Length()) {
    ...
    default:
      info.GetIsolate()->ThrowException(
          v8::Exception::Error(v8::String::NewFromUtf8(info.GetIsolate(), "too many arguments").ToLocalChecked()));
...
py::object CJavascriptObject::Wrap(v8::Local<v8::Value> value, v8::Local<v8::Object> self) {
  ...
  if (value->IsDate()) {
    ...
    return py::object(
        py::handle<>(::PyDateTime_FromDateAndTime(t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min,
...
void CJavascriptArray::LazyConstructor(void) {
  ...
  if (m_items.is_none()) {
    ...
    for (Py_ssize_t i = 0; i < (Py_ssize_t)m_size; i++) {
      array->Set(context, v8::Uint32::New(isolate, i),
                 CPythonObject::Wrap(py::object(py::handle<>(py::borrowed(PyList_GET_ITEM(m_items.ptr(), i))))));
    }
    ...
    for (Py_ssize_t i = 0; i < (Py_ssize_t)m_size; i++) {
      array->Set(context, v8::Uint32::New(isolate, i),
                 CPythonObject::Wrap(py::object(py::handle<>(py::borrowed(PyTuple_GET_ITEM(m_items.ptr(), i))))));
    }
    ...
    while (NULL != (item = ::PyIter_Next(iter.ptr()))) {
      array->Set(context, v8::Uint32::New(isolate, m_size++),
                 CPythonObject::Wrap(py::object(py::handle<>(py::borrowed(item)))));
    }
  }
    ...
    if (values) {
      ...
      if (0 == PySlice_GetIndicesEx(PySlice_Cast(key.ptr()), arrayLen, &start, &stop, &step, &sliceLen)) {
        ...
        for (Py_ssize_t idx = 0; idx < itemSize; idx++) {
          Object()->Set(context, (uint32_t)(start + idx * step),
                        CPythonObject::Wrap(py::object(py::handle<>(py::borrowed(items[idx])))));
        }
      }
    }
  } else if (PyInt_Check(key.ptr()) || PyLong_Check(key.ptr())) {
